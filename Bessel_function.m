...
bessel_zeros = zeros(num_roots, num_functions);
for col = 1: num_functions
  n = col - 1 ;
  for row = 1:num_roots
    j = @(x , n, theta) (1/pi)* cos(x*sin(theta) - n * theta ); %The J_i(x) function
    bzeros(row,n+1) = fzero( @(x) integral( @(theta) j(x, n, theta), 0, pi), zeros_guess(row,n+1)); %store the zeros of J_i(x) into the i_th column
  end
end 
