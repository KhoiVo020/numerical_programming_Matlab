...
for i=1:nx %iterate in x   
  for j=1:nz    %iterate in z
    error=1000;    
    Y=y0;               
    while error > max(RelTol ,AbsTol)
      J=[-sigma, sigma, 0; r-Z(i, j), -1, -X(i, j); Y, X(i, j), -beta]; %Jacobian matrix
      rhs = -[sigma*(Y-X(i, j)); r*X(i, j)-Y-X(i, j)*Z(i, j); X(i, j)*Y-beta*Z(i, j)];%right hand side
      delta_xyz=J\rhs; %$solve for delta_x delta_y delta_z
      X(i, j) = X(i, j) + delta_xyz(1); %update X=x_n + delta_x
      Y = Y + delta_xyz(2); %update Y=y_n + delta_y
      Z(i, j) = Z(i, j) + delta_xyz(3); %update Z=z_n + delta_z
      error=max(abs(delta_xyz)); %check error again
    end
  end
end
