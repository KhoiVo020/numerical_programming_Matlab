...
for i = 1:n_mu %loop over mu pixel
    x = x_0; % initialize x for each mu value
    for j = 1:n_trans %loop over transient
        x = mu(i) * x * (1 - x); % transient iterations
    end
    for j = 1:n_data %collect data
        x = mu(i) * x * (1 - x); % iterations to collect data
        x_data(j, i) = x; % store the x values
    end
end    
